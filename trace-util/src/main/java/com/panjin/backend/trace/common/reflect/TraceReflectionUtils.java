 ...
 */
package com.panjin.backend.trace.common.reflect;
...
import com.panjin.backend.trace.common.utils.TraceArrays;

/**
 * 反射工具类. 提供访问私有变量,获取泛型类型Class, 提取集合中元素的属性, 转换字符串到对象等Util函数.
 *
 * @author panjin
 * @version $Id: TraceReflectionUtils.java 2016年7月21日 下午5:20:21 $
 */
public abstract class TraceReflectionUtils {

    private static Logger logger = LoggerFactory.getLogger(TraceReflectionUtils.class);
    ...
    public static List<Field> getNonStaticFields(Class<?> clazz) {
        ...
        if (TraceArrays.isEmpty(declaredFields)) {
        ...
        Collections.sort(resultFields, comparator); // 保持固定顺序
        return resultFields;
    }
